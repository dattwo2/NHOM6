...
#
#         resp_obj.append((result_df, source_region))
#
#     # -----------------------------------
#
#     toc = time.time()
...
import pandas as pd
from tqdm import tqdm
from deepface import functions, represent, dst
...
def find(
    ...
    """
    This function applies verification several times and finds identities in a database.

    Parameters:
        img_path (str or np.ndarray): The image path or numpy array.
        db_path (str): Folder path containing images for database.
        model_name (str): Model name for comparison (e.g., "VGG-Face").
        distance_metric (str): Distance metric to use for comparison.
        ...
        normalization (str): Normalization technique for images.
        silent (bool): Whether to suppress logs and progress bars.

    Returns:
        List of pandas DataFrame: Contains the comparison results between the source and database images.
    """
    ...
    if not os.path.isdir(db_path):
        raise ValueError(f"Passed db_path does not exist: {db_path}")
    ...
    # Check if representations file exists
    if os.path.exists(f"{db_path}/{file_name}"):
        if not silent:
            print(f"Representations for images in {db_path} were previously stored in {file_name}.")

        with open(f"{db_path}/{file_name}", "rb") as f:
            representations = pickle.load(f)

            # Ensure the loaded representations have correct structure
            if len(representations) > 0 and len(representations[0]) != len(df_cols):
                raise ValueError(f"The existing {file_name} file is outdated. Please delete it and re-run.")

    else:
        # Create representations from scratch
        employees = []
        for r, _, files in os.walk(db_path):
            for file in files:
                if file.lower().endswith(('.jpg', '.jpeg', '.png')):
                    employees.append(os.path.join(r, file))

        if len(employees) == 0:
            raise ValueError(f"There are no valid image files in {db_path}. Ensure .jpg or .png files are present.")

        representations = []
        pbar = tqdm(range(len(employees)), desc="Finding representations", disable=silent)

        for employee in pbar:
            img_objs = functions.extract_faces(
                img=employees[employee],
                target_size=target_size,
                detector_backend=detector_backend,
                grayscale=False,
                enforce_detection=enforce_detection,
            ...
            for img_content, img_region, _ in img_objs:
                embedding_obj = represent(
                    ...
                    model_name=model_name,
                    enforce_detection=enforce_detection,
                    detector_backend="skip",  # Skip face detector on the image itself
                    align=align,
                    normalization=normalization,
                )

                img_representation = embedding_obj[0]["embedding"]
                instance = [employees[employee], img_representation, img_region["x"], img_region["y"], img_region["w"],
                            img_region["h"]]
                representations.append(instance)

        # Store the representations for future use
        with open(f"{db_path}/{file_name}", "wb") as f:
            pickle.dump(representations, f)

        if not silent:
            print(f"Representations stored in {db_path}/{file_name}.")

    # Create DataFrame from the representations
    df = pd.DataFrame(representations, columns=df_cols)

    # Extract faces from the source image
    source_objs = functions.extract_faces(
        img=img_path,
        target_size=target_size,
        detector_backend=detector_backend,
    ...
    toc = time.time()
    if not silent:
        print(f"find function lasted {toc - tic} seconds.")
